...
a[8]=0;
a[9]=0;
//COMMAND-CONTROL BIT ON NORMAL ADDRESS FIELDS
//HAS-BEEN-REPEATED FIELD ON REPEATER ADDRESS FIELDS CLEARED WHEN PASSED THROUGH THAT REPEATER
control=(b[6]&0x80);
...
void sendclient(uint8_t *data,ssize_t total){
ssize_t bytes;
ssize_t thisblock;
ssize_t sent;
int flags;
if(total==0)total=strlen(data);
bytes=0;
sent=0;
...
int n;for(n=0;n<20;n++)sendclient(binarybuf,strlen(binarybuf));
